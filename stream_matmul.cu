...
__global__ void matrixFMA(float *A, float *B, float *C, int numARows, int numAColumns, int numBColumns)
{
    ...
    {
        float sum = C[row * numBColumns + col]; // Iniciar sum con el valor correspondiente de C
...
void matrixFMACPU(float *A, float *B, float *C, int numARows, int numAColumns, int numBColumns)
{
...
int main()
{
    ...
    for (int i = 0; i < M * N; ++i)
    {
        h_C[i] = rand() / (float)RAND_MAX; // Inicializar la matriz C con ceros
    }

    // Multiplicación de matrices en CPU
    float *h_C_CPU = (float *)malloc(sizeof(float) * M * N);
    matrixFMACPU(h_A, h_B, h_C_CPU, M, K, N);
    ...
    for (int i = 0; i < NUM_STREAMS; ++i)
    {
        ...
        matrixFMA<<<dimGrid, dimBlock, 0, streams[i]>>>(d_A + offset, d_B, d_C + offset, subM, K, N);
